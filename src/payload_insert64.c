...
{
	Elf64_Off injection_offset = ((payload_info64 *) info->payload)->insertion_header.p_offset + ((payload_info64 *) info->payload)->insertion_header.p_filesz;
	Elf64_Shdr *sh_headers = executable->addr + ((payload_info64 *) info->payload)->main_header_replace.e_shoff;
	for (int i = 0; i < ((payload_info64 *) info->payload)->main_header_replace.e_shnum; i++)
	{
